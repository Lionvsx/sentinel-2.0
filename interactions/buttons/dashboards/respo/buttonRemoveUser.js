...
module.exports = class RemoveUserButtonInteraction extends BaseInteraction {
    ...
    async run(client, interaction, buttonArgs) {
        ...
        const summaryEmbed = new MessageEmbed()
            ...
            .addField('<:check:1137390614296678421> UTILISATEURS RETIRES', `\`\`\`${staffResults.length > 0 ? staffResults.join('\n'): 'Aucun'}\`\`\``, false)
            .addField(`<:x_:1137419292946727042> UTILISATEURS INTROUVABLES SUR LE SERVEUR`, `\`\`\`${userErrors.length > 0 ? userErrors.join(',\n') : 'Aucun'}\`\`\``, false)
            .addField(`<:info:1137425479914242178> ROLES RESTANTS`, `Pensez à retirer les rôles sous-pôle ainsi que le divider \`---SOUS POLES---\` des utilisateurs retirés.`, false)
            .setColor('2b2d31')
...
function staffUsers(audience, tempMsg, loading, category, allRoles) {
    return new Promise(async (resolve) => {
        ...
        if (success.length === audience.length) {
            await tempMsg.edit(`**<:check:1137390614296678421> | **Retrait des utilisateurs terminé`)
            resolve(success)
        }
    })
}
